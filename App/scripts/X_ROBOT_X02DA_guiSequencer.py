...
class guiSequencer(wx.Frame):
    ...
    def __init__(self, parent):
        ...
        thread.start_new_thread ( self.threadSequencerStatus,(self.dad.myRobotScript,) )
        
    def robotScriptThread(self):
        globals()['guiSequencerScriptDone']=0
        self.dad.myRobotScript.executeScript()
        globals()['guiSequencerScriptDone']=1
    ...
    def threadSequencerStatus(self,myRobotScript):
        while not globals()['guiSequencerScriptDone']:
            statusText=myRobotScript.chFeedback.getVal()
            pauseVal=myRobotScript.chPause.getVal()
            linStatus=myRobotScript.chLineFeedback.getVal()
            de=DataEvent([statusText,pauseVal,linStatus])
            globals()['guiSequencerScriptPostHandled']=0
            wx.PostEvent(self,de)
            while not globals()['guiSequencerScriptPostHandled']:
                time.sleep(0.50)
    def dispSequencerStatus(self,event):
        [statusText,pauseVal,linStatus]=event.argBundle
        ...
        if type(linStatus)==type(' '):
            ...
            self.txtScript.SetSelection(0,0)
        #print statusText
        globals()['guiSequencerScriptPostHandled']=1
    def OnButBeginButton(self, event):
        if globals()['guiSequencerScriptDone']: # if the script has finished start again
            thread.start_new_thread(self.robotScriptThread,())
            thread.start_new_thread ( self.threadSequencerStatus,(self.dad.myRobotScript,) )
    ...
    def OnButResetButton(self, event):
        ...
        if globals()['guiSequencerScriptDone']:
