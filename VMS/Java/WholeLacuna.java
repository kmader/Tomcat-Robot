...
public class WholeLacuna {
	static Vector<Integer> AlreadyFound;
	static VirtualAim mapA;
	public static void main(String[] args) {
		boolean writeHeader=false;
		String kVer="001_090906";
		System.out.println(" Kevin's Ganz Lacuna Morphometric Analysis v"+kVer);
		...
		try {
			myDB.UpdateRecord("LACGZ-"+myDB.VMSCleanName(args[2]),"Run Started @ "+new Date()+"\n",false,"H");
		} catch (Exception e) {
		...
		if ((mapA.ischGuet) && (disA.ischGuet))  {
			...
			try {
				String dbOutString="";
				dbOutString+="WholeLacuna Version:"+kVer+"\n";
				...
				myDB.UpdateRecord("LACGZ-"+myDB.VMSCleanName(args[2]),dbOutString,"C");
			} catch (Exception e) {
				System.out.println("TomcatDB Problem");
			}
		...
		try {
			...
			myDB.UpdateRecord("LACGZ-"+myDB.VMSCleanName(args[2]),outString,"N");
		} catch (Exception e) {
			System.out.println("TomcatDB Problem");
		}
